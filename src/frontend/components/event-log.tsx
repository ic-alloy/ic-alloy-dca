...
function OkMessage({ logItem }: { logItem: LogItem }) {
  ...
  switch (label) {
    ...
    case "Transfer": return <TxLink tx={logItem.ok[0]} />
  }
}
...
function Label({ logItem }: { logItem: LogItem }) {
  const labelColour = logItem.err.length > 0 ? "bg-destructive" : "bg-primary/50";

  return <div className={cn(labelColour, "inline px-[0.3rem] py-[0.2rem] rounded-sm mr-1 text-foreground")}>{Object.keys(logItem.event)[0]}</div>
}
...
export function EventLog() {
  ...
  if (eventLog.length === 0) {
    return <section className="w-full text-foreground text-sm max-w-[400px]">
      This agent has not been started yet.
    </section>
  }

  return <section className="flex flex-col gap-5 w-full text-foreground text-sm max-w-[400px]">
