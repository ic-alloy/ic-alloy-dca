...
function CanisterAddress() {
  const { data: canister_address, isPending: isFetchingCanisterAddress } = useCanisterAddress();

  if (isFetchingCanisterAddress || !canister_address) {
    return <Skeleton className="h-[19px] w-[125px] inline-block" />
  }

  return <a href={`https://sepolia.etherscan.io/address/${canister_address}`} target="_blank">
...
export function Settings() {
  ...
  if (isFetchingSettings || !settings) {
    return null;
  }
